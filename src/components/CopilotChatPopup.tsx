...
const WEBCHAT_CSS = `
  ...
  /* Hide WebChat's own children */
  .webchat__typing-indicator > * { display: none !important; }

  /* Three real animating dots via pseudo-elements + box-shadow trick */
  ...
  /* dot 1 (leftmost — ::before with box-shadow for dot 2) */
  .webchat__typing-indicator::before {
    animation: dot1 1.2s infinite ease-in-out !important;
    /* dot 2 sits 14px to the right via box-shadow */
    box-shadow: 14px 0 0 0 #9ca3af !important;
  }

  /* dot 3 (rightmost — ::after) */
  .webchat__typing-indicator::after {
    margin-left: 14px !important;
    animation: dot3 1.2s infinite ease-in-out !important;
  }
  ...
  /* dot 2 can't be keyframed via box-shadow alone, so we animate ::before slightly offset */
  @keyframes dot3 {
    0%, 60%, 100% { transform: translateY(0);    opacity: 0.4; }
...
export default function CopilotChatPopup() {
  ...
  const store = useMemo(() => {
    return createStore({}, () => (next: WebChatNext) => (action: WebChatAction) => {
      ...
      if (action.type === 'DIRECT_LINE/INCOMING_ACTIVITY') {
        ...
        // Allow typing events — WebChat renders its built-in indicator for these
        ...
        if (activity?.type === 'message' && activity?.from?.role !== 'user') {
          ...
          const hasRenderableContent = text.length > 0 || attachments.length > 0 || suggested.length > 0;
          const hasRealChars = /[A-Za-z0-9\u00C0-\u024F]/.test(text);
          const isPlaceholder =
            attachments.length === 0 && suggested.length === 0 &&
            !hasRealChars && text.length <= 15;
  ...
  const handleClearChat = () => { setChatEnded(true); setHasInteracted(false); setErr(null); setLoading(false); setToken(null); };
  const startNewChat    = () => { setChatEnded(false); setHasInteracted(false); setErr(null); setToken(null); setSessionId(s => s + 1); };

  return (
    <>
      {!open && (
        <button type="button" onClick={() => setOpen(true)}
          className="fixed bottom-10 right-8 z-[9999]" aria-label="Open chat">
          <span className="flex sm:hidden w-14 h-14 rounded-full bg-white items-center justify-center ring-[3px] ring-[#cca249] shadow-lg">
            <img src="/logo.ico" alt="Lock It Lending" className="w-9 h-9 rounded-full object-cover" />
          </span>
        ...
        <div className={[
          ...
          <div className="flex items-center justify-between border-b px-4 py-3 flex-shrink-0">
            <div className="flex items-center gap-3">
              <div className="h-9 w-9 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden">
                <img src="/logo.ico" alt="Lock It Lending" className="h-9 w-9 rounded-full object-cover" />
              </div>
              <div className="font-semibold">Lock It Lending Assist</div>
            </div>
            <div className="flex items-center gap-2">
              <button type="button" onClick={handleMinimize}  className={ICON_BTN} aria-label="Minimize" title="Minimize">–</button>
              <button type="button" onClick={handleClearChat} className={ICON_BTN} aria-label="Clear chat" title="Clear chat">✕</button>
            </div>
          </div>
          ...
          {chatEnded ? (
            <div className="flex flex-col flex-1 min-h-0">
              <div className="flex-1 flex items-center justify-center text-sm text-gray-500">Chat ended</div>
              <div className="border-t p-4 flex justify-center flex-shrink-0">
                <button type="button" onClick={startNewChat} className={[CTA_BTN, 'text-lg px-8 py-4'].join(' ')}>Start new chat</button>
              </div>
            </div>
          ) : (
            <div className="flex flex-col flex-1 min-h-0">
              {loading && <div className="p-4 text-sm text-gray-600 flex-shrink-0">Loading chat…</div>}
              ...
              {!hasInteracted && !loading && !err && (
                <div className="flex flex-col gap-3 px-4 pt-5 pb-2 flex-shrink-0">
                  <p className="text-center text-xs text-gray-500 px-6">
                    This chat is AI-powered. Chats are recorded for quality using third party services. Learn more from our{' '}
                  ...
                  <div className="flex justify-start">
                    <div className="max-w-[85%] rounded-2xl bg-gray-100 px-4 py-3 text-sm text-gray-900 shadow-sm">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        <button type="button" onClick={() => sendQuick('What loan programs do you offer?')} className={OPTION_BTN}>Loan programs</button>
                        <button type="button" onClick={() => sendQuick('How much home can I afford?')} className={OPTION_BTN}>Affordability</button>
                        <button type="button" onClick={() => sendQuick('What documents do I need to get pre-approved?')} className={OPTION_BTN}>Pre-approval docs</button>
                      </div>
                    </div>
                  </div>
                </div>
