    ...
    <div className="font-sans">
      ...
      <main>
        ...
        <section className="bg-[#f7fbfd] py-16 px-4">
          ...
          <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-center gap-8">
            {teamList.map(team => (
              <div key={team.slug} className="flex-1">
                <Link
                  to={`/team/${team.slug}`}
                  className="rounded-xl overflow-hidden shadow-md group relative block h-full"
                >
                  <img
                    src={team.image}
                    alt={team.name}
